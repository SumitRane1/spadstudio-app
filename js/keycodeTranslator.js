...
//
// ═══ FIX (2026-08-24) ═══
// Added C_SLEEP, C_FAST_FORWARD, C_REWIND to CONSUMER_USAGE — these were
// missing, which caused zmkStringTranslator.js's translate() to fail for
// any layer using &kp C_SLEEP (e.g. the real .keymap's System layer),
// incorrectly forcing a full-rebuild fallback for a perfectly normal key.
//
// USB HID Consumer page (0x0C) usage IDs per the official HID Usage Tables:
//   Sleep = 0x32, Fast Forward = 0xB3, Rewind = 0xB4
//
// This is a DIFFERENT numbering system than your old keycodes.js!
...
const KeycodeTranslator = (() => {
  ...
  const KNOWN_BUILTIN_BEHAVIORS = new Set([
    'Key Press', 'Momentary Layer', 'Toggle Layer', 'To Layer',
    'Transparent', 'None', 'Layer Tap', 'Mod-Tap', 'Sticky Key', 'Sticky Layer',
  ]);


  // ════════════════════════════════════════
  //  KEYBOARD USAGE TABLE (page 0x07)
  // ════════════════════════════════════════
  ...
  // ★ FIX: added C_SLEEP (0x32), C_FAST_FORWARD (0xB3), C_REWIND (0xB4).
  const CONSUMER_USAGE = {
    ...
    C_SLEEP: 0x32, C_FAST_FORWARD: 0xB3, C_REWIND: 0xB4,
  };
  ...
  const FRIENDLY_NAMES = {
    ...
    C_SLEEP: 'Sleep', C_FAST_FORWARD: 'Fast Forward', C_REWIND: 'Rewind',
  };
  ...
  function buildCustomBehaviorBinding(behaviorId) {
    return { behavior_id: behaviorId, param1: 0, param2: 0 };
  }


  // ════════════════════════════════════════
  //  CUSTOM MACRO DISCOVERY
  // ════════════════════════════════════════

  function listCustomBehaviors() {
    return StudioRpc.getAllCachedBehaviors()
      .filter(b => !KNOWN_BUILTIN_BEHAVIORS.has(b.display_name));
  }
  ...
  function listMacroOptions() {
    return listCustomBehaviors().map(b => ({
      value: b.id,
